...
Flask前后端分离API后台接口的实现demo，前端可以接入小程序，目前后台已经完成API基础架构，使用`postman`调试.

## 收获

1. 我们可以接受定义时的复杂，但不能接受调用时的复杂
2. 如果你觉得写代码厌倦，无聊，那你只是停留在功能的实现上，功能的实现很简单，你要追求的是更好的写法，抽象的艺术，不是机械的劳动而是要**创造**
3. `Sqlalchemy`中对类的创建都是用元类的方式，所以调用的时候都不用实例化，当我们重写`__init__`方法是需要调用`orm.reconstrcut`装饰器，才会自行类实例化的构造函数
